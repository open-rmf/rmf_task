...
public:
  ...
  Estimate(std::optional<State> finish_state, rmf_traffic::Time wait_until);

  /// Finish state of the robot once it completes the request.
  std::optional<State> finish_state() const;

  /// Sets a new finish state for the robot.
  Estimate& finish_state(std::optional<State> new_finish_state);
